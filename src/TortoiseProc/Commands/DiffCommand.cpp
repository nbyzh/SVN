...
// Copyright (C) 2007-2008, 2010-2011, 2013-2015, 2018, 2021-2022, 2024 - TortoiseSVN
    ...
    if (path2.empty())
    {
        ...
        if (parser.HasKey(L"startrev") && parser.HasKey(L"endrev"))
        {
            ...
            if (parser.HasVal(L"pegrev"))
                pegRevision = SVNRev(parser.GetVal(L"pegrev"));
            else if (parser.HasVal(L"pegrevision"))
        ...
        else
        {
            ...
            if (parser.HasKey(L"unified"))
            {
                SVNRev pegRevision;
                if (parser.HasVal(L"pegrev"))
                    pegRevision = SVNRev(parser.GetVal(L"peg"));
                else if (parser.HasVal(L"pegrevision"))
                    // Support deprecated option
                    pegRevision = SVNRev(parser.GetVal(L"pegrevision"));
                CString diffOptions;
            ...
            else
            {
                ...
                {
                    bRet = diff.DiffFileAgainstBase(GetExplorerHWND(), cmdLinePath, baseRev, ignoreProps);
                }
            }
        }
    }
