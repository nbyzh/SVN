...
// Copyright (C) 2008-2011, 2013-2014, 2018, 2021, 2024 - TortoiseSVN
...
bool ShowCompareCommand::Execute()
{
    ...
    if (parser.HasVal(L"pegrev"))
        pegRev = SVNRev(parser.GetVal(L"pegrev"));
    else if (parser.HasVal(L"pegrevision"))
        //Support deprecated option
        pegRev = SVNRev(parser.GetVal(L"pegrevision"));
    if (parser.HasVal(L"headpegrev"))
        diff.SetHEADPeg(SVNRev(parser.GetVal(L"headpegrev")));
    else if (parser.HasVal(L"headpegrevision"))
        // Support deprecated option
