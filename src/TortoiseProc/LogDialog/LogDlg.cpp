...
// Copyright (C) 2003-2023, 2025 - TortoiseSVN
                  ...
                  apr_time_t time, const MergeInfo* mergeInfo)
{
    ...
    if (m_limit != 0)
    {
        ...
        if (!m_bMonitoringMode && m_pTaskbarList)
        {
            ...
            int cur    = m_logEntries.size() - m_prevLogEntriesSize - l - m_cMergedRevisionsReceived;
            int total  = u - l;
            // The Windows 11 taskbar has a performance bug, whereby updating the progress value
            // quickly enough with different values causes it to freeze while it tries to asynchronously
            // process the new values one by one. In our case, showing a log with thousands of entries
            // can cause the taskbar to freeze for several seconds or even minutes!
            // For this reason, we remap the progress range to [0,100], as calling SetProgressValue()
            // with the same value multiple times is ok, and bypasses the slowness.
            int cur100 = 100;
            if (total > 0)
                cur100 = static_cast<int>(static_cast<long long>(cur) * 100 / total);
            m_pTaskbarList->SetProgressState(m_hWnd, TBPF_NORMAL);
            m_pTaskbarList->SetProgressValue(m_hWnd, cur100, 100);
        }
    }
    else if (m_startRev.IsNumber() && m_endRev.IsNumber())
    {
        if (!logItem->HasParent() && rev < m_tempRev)
        {
            ...
            if (!m_bMonitoringMode && m_pTaskbarList)
            {
                int l, u;
                m_logProgress.GetRange(l, u);
                int cur    = static_cast<svn_revnum_t>(m_startRev) - rev + static_cast<svn_revnum_t>(m_endRev) - l;
                int total  = u - l;
                // remap the progress range to [0,100] - see above
                int cur100 = 100;
                if (total > 0)
                    cur100 = static_cast<int>(static_cast<long long>(cur) * 100 / total);
                m_pTaskbarList->SetProgressState(m_hWnd, TBPF_NORMAL);
    ...
    for (auto& item : m_monitorItemListForThread)
    {
        ...
        if (item.parentPath)
        {
            if ((item.lastChecked + (max(item.minMinutesInterval, item.interval) * 60LL)) < currentTime)
            {
                ...
                auto decrUsername = CStringUtils::Decrypt(item.userName);
                auto decrPassword = CStringUtils::Decrypt(item.password);
                callback->SetAuthData(decrUsername ? decrUsername.get() : L"", decrPassword ? decrPassword.get() : L"");
        ...
        else
        {
            ...
            if ((item.lastChecked + (max(item.minMinutesInterval, item.interval) * 60LL)) < currentTime)
            {
                ...
                CString      decrUsername = CStringUtils::Decrypt(item.userName).get();
                CString decrPassword = CStringUtils::Decrypt(item.password).get();
                svn.SetAuthInfo(decrUsername, decrPassword);
