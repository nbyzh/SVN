...
// Copyright (C) 2003-2021, 2023, 2025 - TortoiseSVN
...
{
    // Custom comparator to sort directories before files and parent directories before their children
    auto comparator = [](const CTSVNPath& left, const CTSVNPath& right) {
    ...
    // Sort paths using the custom comparator
    std::sort(m_paths.begin(), m_paths.end(), comparator); // Remove the duplicates
...
#    endif
    ...
    static void RemoveChildrenTest()
    {
        ...
        list = {};
        list.AddPath(CTSVNPath(L"D:\\bar"));
        list.AddPath(CTSVNPath(L"D:\\bar.txt"));
        list.AddPath(CTSVNPath(L"D:\\bar\\blah"));
        ATLASSERT(list.GetCount() == 3);
        list.RemoveChildren();
        ATLASSERT(list.GetCount() == 2);
    }
